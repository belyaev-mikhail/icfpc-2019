...
fun SimulatingCloningBotSwarm(simref: MutableRef<Simulator>, points: Set<Point>, idx: Int = 0) = sequence {
    val commands = mutableMapOf<Int, Sequence<Pair<Int, Command>>>()

    commands[idx] = SimulatingCloningBot(simref, points, idx)
    ...
    while (true) {
        ...
        for (activeId in activeIdx) {
            val botCommands = commands.getOrPut(activeId) { theMostSmartestPrioritySimulatingAstarBot(simref, points, activeId) }
...
fun SimulatingCloningBot(simref: MutableRef<Simulator>, points: Set<Point>, idx: Int) =
        sequence {
            while (true) {
                val sim by simref
        ...
        }.withIdx(idx) + theMostSmartestPrioritySimulatingAstarBot(simref, points, idx)
